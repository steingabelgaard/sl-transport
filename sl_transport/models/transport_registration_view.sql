...
left outer join (select registration_id, min(TO_DATE(day,'YYYY_MM_DD'))as in_date , max(TO_DATE(day,'YYYY_MM_DD')+1) as out_date from event_registration_day where participation in ('volunteer', 'relative') group by registration_id) cdf on cdf.registration_id= er.id
